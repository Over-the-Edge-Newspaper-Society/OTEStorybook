...
interface EventDialogCardProps {
  event: Event;
  eventMetadata: Record<string, EventMetadata>;
  showCost?: boolean;
}

export function EventDialogCard({ event, eventMetadata, showCost = true }: EventDialogCardProps) {
  ...
  return (
    <div className="max-w-2xl w-full bg-card border border-border rounded-lg shadow-lg p-4 sm:p-6">
      <div className="flex flex-col space-y-1.5 text-center sm:text-left">
        <h2 className="font-semibold tracking-tight text-xl text-foreground">{event.title}</h2>
        {event.description && (
          <div className="mt-2">
            <p className={`text-sm text-muted-foreground leading-relaxed break-words ${isDescriptionExpanded ? 'max-h-[40vh] overflow-y-auto pr-2' : ''}`}>
