...
const TestimonialCard: React.FC<TestimonialCardProps> = ({
  ...
  const fallbackInitial = testimonial.by?.[0]?.toUpperCase?.() ?? "#";
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isOffscreen && "opacity-0 pointer-events-none",
        isCenter
    ...
    >
      ...
      {variant === "docs" ? (
        <p
          className={cn(
            "absolute top-8 left-8 right-8 mt-2 text-2xl",
        ...
        <div
          className={cn(
            "mb-4 h-16 w-16 font-semibold text-3xl flex items-center justify-center",
            isCenter
...
export const StaggerTestimonials: React.FC<StaggerTestimonialsProps> = ({ items, variant = "default" }) => {
  const [cardSize, setCardSize] = useState(365);
  const baseTestimonials = items?.length ? items : defaultTestimonials;

  // Ensure we have at least 20 items by repeating if necessary
  const minItems = 20;
  const expandedTestimonials = React.useMemo(() => {
    if (baseTestimonials.length >= minItems) {
      return baseTestimonials.map((item, idx) => ({ ...item, tempId: idx }));
    }
    const result: Testimonial[] = [];
    let tempId = 0;
    while (result.length < minItems) {
      for (const item of baseTestimonials) {
        if (result.length >= minItems) break;
        result.push({ ...item, tempId: tempId++ });
      }
    }
    return result;
  }, [baseTestimonials]);

  const [testimonialsList, setTestimonialsList] = useState(expandedTestimonials);
